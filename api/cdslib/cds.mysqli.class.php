 ...
 *   Michael Ansaldi
 *   Ryan Tse
 *   Paul Bailey cirrlus.co.za  additions
 */
...
class MySQL
{
	...
	private $db_port    = "";          // optional character set (i.e. utf8)	
	...
	public function __construct($connect = true, $database = null, $server = null,
								$username = null, $password = null, $port=null, $charset = null) {
	...
	public function AutoInsertUpdate($tableName, $valuesArray, $whereArray) {
		...
		//pb 2020-01-15, old version was to select ALL columns which slows down the query, 
    //but not needed as we only need a dummy col to speed it up, we dont need more than 1 rec returned and dont need sorting.       
    // this should make the query speed optimised to check if a record exists		
		if (! $this->Error()) {			
			$hasrecs = $this->RecordExists($tableName, $whereArray);			
			if (! $this->Error()) {				
				if ($hasrecs) {
	...
	// NBNBNBNBNBNB This function should not be used
	/**
	 * Automatically does an INSERT or UPDATE depending if an existing record
	 * exists in a table. Will not update if row is not unique.
	 *
	 * @param string $tableName The name of the table
	 * @param array $valuesArray An associative array containing the column
	 *                            names as keys and values as data. The values
	 ...
	 * @return boolean Returns TRUE on success or FALSE on error
     */
    public function AutoInsertSingleUpdate($tableName, $valuesArray, $whereArray) {
      
      $this->ResetError();           
      
      //pb 2020-01-15, old versionwas to select ALL columns which slows down the query, 
      //but not needed as we only need a dummy col to speed it up, we dont need more than 1 rec returned and dont need sorting.       
      // this should make the query speed optimised
      
  		$this->SelectRows($tableName, $whereArray,array("'1' as tempcol"),null,true,1); 
        if (! $this->Error()) {
			if ($this->HasRecords()) {
                if ($this->last_result->num_rows === 1) {
                    $result = "Updating single row <br>";
                    echo $result;
                    //return $this->UpdateRows($tableName, $valuesArray, $whereArray);
                } else {
                    $this->SetError("Update row not unique. Not updating.", -1);
                    return false;
                }
			} else {
                $result = "Inserting row <br>";
                echo $result;
				//return $this->InsertRow($tableName, $valuesArray);
			}
		} else {
	...
	/**
 * Automatically does a BULK INSERT or UPDATE on multiple rows
	 * exists in a table
	 *
	 * @param string $tableName The name of the table
	 * @param array $valuesArray An associative array containing the column
	 ...
	 * @return boolean Returns TRUE on success or FALSE on error
	 */
	public function AutoInsertOrUpdateAllRecords($tablename, $datasetvaluesArray, $wherecolumns) {
		
		$columns = explode(',',$wherecolumns);
				
		for($a = 0; $a < count($datasetvaluesArray); $a++){

			$whereArray = array();		
			$row = $datasetvaluesArray[$a];			
			for($b = 0; $b < count($columns); $b++){
				$fn = trim($columns[$b]);
				if ($fn !== '') {
					$whereArray[$fn] = $row[$fn];	
				}				
			}	
			
			$this->AutoInsertUpdate($tablename, $row, $whereArray);
			if ($this->Error()) {
				return $this->Error(); 
			}
		}
	 ...
	 *                            must be SQL ready (i.e. quotes around
	 *                            strings, formatted dates, ect)
	 * @param boolean $insertIgnore DOes an insert ignore
	 * @return string Returns a SQL INSERT statement
	 */
	static public function BuildSQLInsert($tableName, $valuesArray, $insertIgnore = false) {
		$columns = self::BuildSQLColumns(array_keys($valuesArray));
		$values  = self::BuildSQLColumns($valuesArray, false, false);
		$sql = "INSERT ".($insertIgnore ? "IGNORE" : "")." INTO `" . $tableName .
...
static public function GetSQLSelect($tableName, $columns = null, $whereArray = null, 
		...
		if (! is_null($sortColumns)) {
			$sql .= " ORDER BY " .$sortColumns." " . ($sortAscending ? "ASC" : "DESC");
		}
		return $sql;
	}
	...
	public function Close() {
		...
		if ($success && isset($this->mysql_link) ) {
			try {
   ...
   /**
	 ...
	 */
	public function GetHTML($showCount = true, $styleTable = null, $styleHeader = null, $styleData = null, $classId='', $rowHeaderId='', $rowId='', $cellId = '', $escapehtml = true) {
		...
		if ($this->last_result) {
			if ($this->RowCount() > 0) {
				...
				$html .= "<table $ci style=\"$tb\" cellpadding=\"2\" cellspacing=\"2\">\n";
				$this->MoveFirst();
				$header = false;
				while ($member = mysqli_fetch_object($this->last_result)) {
					if (!$header) {
						$html .= "\t<thead>\t<tr ".$rowHeaderId." >\n";
						foreach ($member as $key => $value) {
							$html .= "\t\t<td ".$cellId." style=\"$th\"><strong>" . ($escapehtml ? htmlspecialchars($key) : $key) . "</strong></td>\n";
						}
						$html .= "\t</tr>\t</thead>\n";
						$header = true;
					}
					$html .= "\t<tr ".$rowId.">\n";
					foreach ($member as $key => $value) {
						$html .= "\t\t<td ".$cellId." style=\"$td\">" . ($escapehtml ? htmlspecialchars($value) : $value) . "</td>\n";
					}
					$html .= "\t</tr>\n";
				}
				$this->MoveFirst();
	...
	/**
	 * This function returns the last query as a Styled HTML table
	 *
	 * @param boolean $cssContent provide css if needed,
	...
	public function GetStyledHTML($cssContent = '', 
																 $cssTable = '',
																 $cssHeader = '',
																 $cssCells = '',
																 $cssRow = '',																	 
		...
		if ($this->last_result) {
				...
				$html .= '<table '.($cssTable ? 'class="'.$cssTable.'"' : '').'>'.PHP_EOL;
				$this->MoveFirst();
				...
				$this->MoveFirst();
				if ($showFooter) {
					$footertext = $footertext ? $footertext : 'Record Count: '.($this->RowCount() > 0 ? $this->RowCount() : 0);
	...
	/**
* Returns the last query as a CSV document
	*
	* @return string CSV containing all records listed
	*/
	public function GetCSV($delimiter = ',',$enclosequotes = '', $showheader = true) {
		if ($this->last_result) {
			if ($this->RowCount() > 0) {
				$csv = '';
				$header = false;
				...
				while ($member = mysqli_fetch_object($this->last_result)) {
					...
					if (!$header) {
						...
						if ($showheader) {   
							$csv .= implode($delimiter, $cols) . PHP_EOL;							
						}						
						$header = true;
					}
					...
					$cols = array();			
					foreach ($member as $key => $value) {
							$cols[] = $enclosequotes.$value.$enclosequotes;
					}
					$csv .= implode($delimiter, $cols) . PHP_EOL;						
				}
			} else {
...
//pb $root->setAttribute('query', $this->last_sql);
...
/**
	 ...
	 */
	public function InsertRow($tableName, $valuesArray, $insertIgnore = false) {
		$this->ResetError();
		if (! $this->IsConnected()) {
			$this->SetError("No connection");
			return false;
			...
			// Execute the query
			$sql = self::BuildSQLInsert($tableName, $valuesArray, $insertIgnore);
			if (! $this->Query($sql)) {
				return false;
			} else {
	...
	/**
	 * Copies a row into a table in the connected database
	 *
	 ...
	 * @param string $wherearray the key/value pairs to define a where clause.

	 * @param array  $excludecolumns An array containing the columns names to exclude when copying
	 * @param array  $replacevalues An key-value array containing the key and value to replace NB: values must NOT be quoted as it is done already
	 * @param array  $mappingsarray An key-value array containing the key (source) field and value (destinationtable) field to copy to, which allows copy to dest table with differnt column names 
	 * @param boolean $escapevalues tur or false to escape values before inserting
	 * @return integer Returns last insert ID on success or FALSE on failure
	 */
	public function CopyRows($sourcetable, $destinationtablename, $wherearray, $excludecolumns = array(), $includenullvalues = false, $replacevalues = array(), $mappingsarray = array(), $escapevalues = false) {
		$this->ResetError();
		if (! $this->IsConnected()) {
			$this->SetError("No connection");
			return false;
			...
			$sql = 'SELECT * from `'.$sourcetable.'` '.self::BuildSQLWhereClause($wherearray);
			...
			foreach ($rows as $row)  {
				...
				foreach ($replacevalues as $key => $value) {   // Replace values that need replacing				
					$valuesArray[$key] = $value;				
				}		

							
				foreach ($excludecolumns as $column) {   // disable the excluded columns				
					unset($valuesArray[$column]);				
				}					

				$insertvaluesArray = $valuesArray;			
				if (!$includenullvalues) {                     // if selected then it does not drop null values from the instert list, default is false as it makes no sense to copy null fields into a new table
					foreach ($valuesArray as $key => $value) {   // strip out null values if asked
						if (is_null($value)) {
							unset($insertvaluesArray[$key]);				
						}				
					}									
				}
							
				if ($mappingsarray) {
				foreach ($mappingsarray as $key => $value) {   // does a mapping for dest table							
	    		if (array_key_exists($key, $insertvaluesArray)) {
			        $insertvaluesArray[$value] = $insertvaluesArray[$key];
			        unset($insertvaluesArray[$key]);
			    } 			    
				}					    
				}
				...
				$insertvaluesArray = $this->quote($insertvaluesArray, $escapevalues, true);    // if null values then allow for no quotes
				if (! $this->InsertRow($destinationtablename, $insertvaluesArray)) {
					return false;
				} else {
			...
			return count($ids) > 1 ? $ids : $ids[0];   // return array or just 1 value
...
/**
	 * Emptyies a table
	 *
	 * @param string $tableName The name of the table to be emptied
	 * CAUTION!!!!!!! Use with care, no rollbacks implemented
	 * @return integer Returns true on success or FALSE on failure
	 */
	public function EmptyTable($tableName) {
		...
		if (! $this->IsConnected()) {
			...
			return $this->DeleteRows($tableName);
		}
	}
	 ...
	 * @param string $port     (Optional) Force specific port 
	 * @return boolean Returns TRUE on success or FALSE on error
	 */
	public function Open($database = null, $server = null, $username = null,
						 $password = null, $port = null, $charset = null, $pcon = false) {
		$this->ResetError();
		...
		if ($this->db_port) {   
			$this->mysql_link = @mysqli_connect (
				$pcon . $this->db_host, $this->db_user, $this->db_pass, "", $this->db_port);
		} else {
			$this->mysql_link = @mysqli_connect (
				$pcon . $this->db_host, $this->db_user, $this->db_pass);
		}
	...
	public function Query($sql) {
		...
		$sql = trim($sql);      // added PB 2019-12-26 if you happend to have spaces before your select statemnt
		$this->ResetError();
		$this->last_sql = $sql;
			
		// flush multi_queries	changed by Paul to prevent Strick Standards error
		do {
   		if (!mysqli_more_results($this->mysql_link)) {
    		break;
   		}
		} while (mysqli_next_result($this->mysql_link));				
		...
		if(! $this->last_result) {
			...
			if (strpos(strtolower($sql), "insert") === 0) {
				...
				return $this->last_result;
			} else if(strpos(strtolower($sql), "call") === 0) {
				$numrows = mysqli_field_count($this->mysql_link);
				if ($numrows > 0) {
					$this->active_row = 0;
				} else {
	...
	/**
 * Executes the given SQL MULTI query and returns the records
	 *
	 * @param string $sql The query string should not end with a semicolon
	 * @return object PHP 'mysql result' resource object containing the records
	 *                on SELECT, SHOW, DESCRIBE or EXPLAIN queries and returns;
	...
	public function MultiQuery($sql) {
		...
		// flush multi_queries	
		while (mysqli_next_result($this->mysql_link)) { if (!mysqli_more_results($this->mysql_link)) break; }
				
		// Now run query		
		$this->last_result = @mysqli_multi_query($this->mysql_link, $sql);
		if(! $this->last_result) {
			$this->active_row = -1;
			$this->SetError();
			return false;
	 ...
	 */
public function QueryArray($sql, $resultType = MYSQLI_ASSOC, $returnHeaders = FALSE) {
	
		$this->Query($sql);
		if (! $this->Error()) {
			return $this->RecordsArray($resultType, $returnHeaders);
		} else {
			return false;
		}
	}
	 ...
	 */
	public function QuerySingleRowArray($sql, $resultType = MYSQLI_ASSOC) {
		$this->Query($sql);
		if ($this->RowCount() > 0) {
	 ...
	 */
	public function RecordsArray($resultType = MYSQLI_ASSOC, $returnHeaders = FALSE) {
		$this->ResetError();
		if ($this->last_result) {
			if (! mysqli_data_seek($this->last_result, 0)) {
				...
				if ($returnHeaders === TRUE) {
					...
					//	$members[0][$value] = $value;     // add fieldnames to header at pos 0
					//}	
				}			
...
//while($member = mysqli_fetch_object($this->last_result)){
   ...
   /**
    ...
    public function escape($string)
    {
            ...
            //PB PHP8 this function has been removed. Thanks very much php8 for letting us know
     ...
     */
    public function implodearray($pieces)
    {
		...
		//pb to do
    public function convertarraytoinclause($arrayvalues)
    {
        $result = '';
        ...
        // iterate through the array's items and "glue" items together
        foreach ($arrayvalues as $piece) 
					$newarray = $this->escape($piece);
     ...
     *                                  Default is FALSE.
     * 
     *  @param boolean $quotevaues			(optional) If set, then each value will be quoted
     * 
     * 								                  Default is FALSE.
     * 
     *  @param int $chunksize						(optional) If > 0, then each the bulk insert will be chunked according to the size
     * 
     * 								                  Default is 0, ie all records will be posed in 1 go.
     * 
     *  @param boolean $updateaswell		(optional) if set, it will do an ON DUPLICATE KEYS updatte, cant be used with $insertignore set
     * 
     *  @since  2.1
     *
     *  @return boolean                 Returns TRUE on success of FALSE on error.
     */
public function InsertBulk($table, $data, $ignore = false, $quotevalues = false, $chunksize = 0, $updateaswell = false, $sqlfix = false)
    {
        // if $data is not an array of arrays
			$poparray = array_values($data);		 
      if (!is_array(array_pop($poparray))) {            

        $dat[] = $data;
        $data = $dat;   // convert into array of array
            ...
            if ($updateaswell) {
								foreach ($columns as $col) {
									$updatekeys[] = '`'.$col.'`= VALUES(`'.$col.'`)';
								}				
								$updateaswell = ' ON DUPLICATE KEY UPDATE  '.implode(',',$updatekeys);
						}
			  			...
			  			foreach	($arrayvalues as $value) {
			  				if (is_null($value)) {
									$arrayv[] = 'null';
                                                                        ...
                                                                        $value = $sqlfix ? $this->SQLFix($value) : $value; // sql fix values if asked
									$value = $quotevalues ? dbquote($value) : $value; // quote values if asked
									$arrayv[] = $value;   
								}
							}
			  			$sqlarray[] = '(' . implode(',',$arrayv).')';
			  			//echo implode(',',$arrayv).'<br>';
			  			
			  			$cnt++;			  			
			  			if (($cnt > $chunksize) && ($chunksize > 0)) {
								
								$sql = $headersql . implode(',',$sqlarray). ' ' . $updateaswell;       
								$result = $this->Query($sql);
								if (!$result) {
									// if script gets here, return false as something must've been wrong
									return false;
								}
								$sql = '';
								$sqlarray = array();
			...
			// check if any chunks left, if so then flush it
			if (count($sqlarray) > 0)	{   
      	$sql = $headersql . implode(',',$sqlarray). ' ' . $updateaswell;       
				$result = $this->Query($sql);	 				// return true if query was executed successfully
			} 				
			
			return $result;  // we return the last $result of any query.  
    ...
    /**
     *  Does a bulk insert or update depending if key values exists
     * !!!!!!!!!!!!!!!NBNBNBNB There must be at least 1 unique key setup, as it uses this to update
     *  @return boolean                 Returns TRUE on success of FALSE on error.
     */
    public function InsertUpdateBulk($table, $data, $quotevalues = false, $chunksize = 0,$sqlfix = false)
    {

			if ($data) {
						
			  return $this->InsertBulk($table, $data, false, $quotevalues, $chunksize, true, $sqlfix);	
...
// flushes result set buffer, used before calling procedures
	public function PurgeResults() {
	...
	*/	
	public function fieldbyname($dataset,$rowno,$fieldname){
	 ...
	 * Kills processes 
	 *
	 * @param integer $timeout kill process if > timeout
	 	 * @return boolean Returns TRUE on success or FALSE on error
	 */	
	public function KillProcesses($timeout = 3600) {
			...
			else 
		{
				...
				{		
						$rowinfo = $row["Id"]." ".$row["db"]." ".$row["Command"]." (".$row["Time"].") ".$row["Info"]."     --> ".$row["Host"]."<br>";
						
						$process_id=$row["Id"];
    				if (($row["User"] != "root" ) && ($row["Time"]>$timeout))
						{       
						  $sql = "KILL ".$process_id;
        			if ($this->Query($sql)) 
								$killedoff .= "Killed Off : ".$rowinfo;																									
    				}		
						else
							$killedoff .= "Ignored    : ".$rowinfo;													
				}  // query
				return  $killedoff;
		}		//error 
	}		 // function
 	...
 	/**
	 * Adds quote to value for sql queries
	 *
	 * @param string value
	 	 * @return quote value
	 */		
	public function dbquote($value) {	
        ...
        public function escapesqli($text, $extra = false)
	{
		$result = mysqli_real_escape_string($this->mysql_link, $text);
		//$result = mysql_real_escape_string($text, $this->mysql_link);
	...
	public function quote($text, $escape = false, $allownulls = false)
	{
		if (is_array($text))
		{
			foreach ($text as $k => $v)
			{
				$text[$k] = is_null($v) && $allownulls ? 'null' : $this->quote($v, $escape);  // if there is a null value then put a null statement
			}
		...
		{
			return '\'' . ($escape ? $this->escapesqli($text) : $text) . '\'';
		}
	}
	
	/**
	 * returns teh mysql link id
	 *
	 */
  ...
  /**
	 ...
	 * @param string $columndatatype, if not supplied defaults to varchar(100)
	 * @param string $defualtvalue, if not supplied defaults to null
	 * @return result true or false
	 */
	public function AddColumn($table, $columnname, $columndatatype = 'VARCHAR(100)', $defaultvalue = 'NULL') {
		$this->ResetError();
		if (! $this->IsConnected()) {
			$this->SetError("No connection");
			return false;
		} else {
			$sql = "ALTER TABLE `{$table}` ADD COLUMN `{$columnname}` {$columndatatype} {$defaultvalue} ";
  ...
  /**
	 ...
	 * @param string $fieldnames is a comma separated list
	 * @param string $fieldvalues is a comma separated list, fieldvalues must be enclosed or escaped if required
	 * @param string $wherefields   //noormally id fieldname, but can be multiple with delimneteres
	 * @param string $wherevalues    // normally id value, but can be multiple with delimneteres
	 * @return result true or false
	 * NB: field values must be enclosed or escaped as required, also fieldnames and fieldvalues must have equal count in csv list
	 */
	public function UpdateFields($tablename, $fieldnames, $fieldvalues, $wherefields, $wherevalues, $fielddelimiter = ',') {
		$this->ResetError();
		if (! $this->IsConnected()) {
			$this->SetError("No connection");
			return false;
			...
			$fnames = explode($fielddelimiter,$fieldnames);
			$fvalues = explode($fielddelimiter,$fieldvalues);
			
			$wfields = explode($fielddelimiter,$wherefields);
			$wvalues = explode($fielddelimiter,$wherevalues);
			
			$setvalues = array();
			for($a = 0; $a < count($fnames); $a++){
				$setvalues[] = "`{$fnames[$a]}` = {$fvalues[$a]} ";
			}
	
			$wherevalues = array();
			for($a = 0; $a < count($wfields); $a++){
				$wherevalues[] = "`{$wfields[$a]}` = {$wvalues[$a]} ";
			}
				
			$setvalues = implode(',', $setvalues);			
			$setwhere = implode(' AND ', $wherevalues);
	...
	/**
	 * Checks if a record exsists
	 *
	 * @param string $tablename required
	 * @param string $wherefields   //noormally id fieldname
	 * @return result true or false
	 */	
	public function RecordExists($tableName, $whereArray) {
		...
		if (! $this->IsConnected()) {
			...
			// we only need to check if a record exists, we domnt need the column or many rows, so this is optimised for speed
			$sql = "SELECT '1' as tempcol FROM `" . $tableName . "`";
			if (is_array($whereArray)) {
				$sql .= self::BuildSQLWhereClause($whereArray);
			}	
			$sql .= " LIMIT 1";
...
/**
	 * Add any fields to db if need be
	 *
	 * @param string $table required
	 * @param array $row the data row
	 * @param string $defaultcol the default datatype if the column needs to be added
	 * @return result true or false
	 */
	public function SyncColumns($table, $row, $defaultcol = 'VARCHAR(255)') {
		$this->ResetError();
		if (! $this->IsConnected()) {
			$this->SetError("No connection");
			return false;
			...
			foreach ($row as $key => $value) {
				if (!in_array($key, $columns)) {
					$this->AddColumn($table, $key, $defaultcol);
				}
			}
		}
		return true;
