 ...
 * Mar 7, 2013 - Added support for other prefixes as well as additional functions
 *
 * @version 3.0
...
class Template {
     ...
     */
    public function __construct($templateFile = '', $prefixtag = self::PREFIX_DEFAULT, $postfixtag = self::POSTFIX_DEFAULT)
    {

				if ($prefixtag !== null) $this->prefix = $prefixtag;
				if ($postfixtag !== null) $this->postfix = $postfixtag;
    ...
    public function hasTag($tagName) {
        ...
        $pattern = '/'.$this->prefix. $tagName . $this->postfix .'/';
				
				$pattern = str_replace($pattern, ' ', '\\s');  // replace any spaces
    ...
    public function addContent($content) {
        if ($content == '') {
            // Do nothing
				 		...
				 		$newcontent = $this->getPage().$content;
            $this->setPage($newcontent);
        }
    }
    ...
    public function removeAllTags()
    {
        ...
        $this->page = preg_replace('/'.$this->prefix.'(.|\s)*?'.$this->postfix.'/', '', $this->page);
    }
    ...
    public function replaceTag($tagName, $content = '', $fixHTML = false, $stripSlashes = false, $keepTag = false)
    {
        if ($tagName == '') {
            ...
            if($keepTag == true) {
                $tag = $this->prefix . $tagName . $this->postfix;
            } else {
                $tag = '';
            }
        }

        $this->setPage(str_replace($this->prefix . $tagName . $this->postfix, $content.$tag, $this->getPage()));
    }

    /**
     * Replaces multiple tags embedded in the template with a string
     *
     * @param string  $tagNames The names of the tag in array [tagName => tagvalue, tagName => tagvalue, ....]
     * @param string  $content The content to insert
     * @param boolean $fixHTML Determines if the content should be formatted
     * @param boolean $stripSlashes Encodes the contect
     * @param boolean $keepTag If true, the tag will stay in the template
     */
    public function replaceMultipleTags($tagNames, $content = '', $fixHTML = false, $stripSlashes = false, $keepTag = false)
    {
        if (!is_array($tagNames)) {
            //do nothing
					...
					foreach ($tagNames as $tagname => $value )	{
						$this->replaceTag($tagname, $value, $fixHTML, $stripSlashes, $keepTag);
					}
    ...
    /**
     * Replaces the prefix replacement tags
     *
     * @param string  $prefix if blank then loads default
     * @param string  $postfix if blank then loads default
		*/
		public function changeTagPrefix($prefix = null, $postfix = null)
    {
			if ($prefix !== null) {
				 $this->prefix = $prefix;
			} else {
				 $this->prefix = Template::PREFIX_DEFAULT;
			}	 
			...
			{  
				$this->postfix = Template::POSTFIX_DEFAULT;
			}	
    }
    ...
    public function replaceTagFromWeb($tagName, $url, $fixHTML = false, $stripSlashes = false, $keepTag = false)
    {
        ...
        if (!$content) {
            ...
            if ($tagName == '') {
                ...
                $this->setPage(str_replace($this->prefix . $tagName . $this->postfix, $content, $this->getPage()));
            }
        }
    }
  ...
  /**
     ...
     * @param boolean $keepTag If true, the tag will stay in the template
     */
    public function replaceTagsFromDataset($dataset, $content='', $fixHTML = false, $stripSlashes = false, $keepTag = false)
    {
        if (!is_array($dataset)) {
            //do nothing
        } else {
					...
					foreach ($dataset as $row)	{					
						foreach ($row as $fieldname => $value )	{
							$this->replaceTag($fieldname, $value, $fixHTML, $stripSlashes, $keepTag);
						}
					}
  ...
  /**
     ...
     * @param boolean $keepTag If true, the tag will stay in the template
     */
    public function replaceTagsFromDatasetRow($datasetrow, $content='', $fixHTML = false, $stripSlashes = false, $keepTag = false)
    {
        if (!is_array($datasetrow)) {
            //do nothing
        } else {
				
					foreach ($datasetrow as $fieldname => $value )	{
						$this->replaceTag($fieldname, $value, $fixHTML, $stripSlashes, $keepTag);
					}
